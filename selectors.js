...
export const getActiveList = state => {
  ...
  if (state.toDoList.toDoList.length) {
    for (let i = 0; i < state.toDoList.toDoList.length; i++) {
      if (~state.toDoList.toDoList[i].name.toLowerCase().indexOf(state.search.searchText.toLowerCase()) && !state.toDoList.toDoList[i].isCompleted && !state.toDoList.toDoList[i].important) {
...
export const getCompletedList = state => {
  ...
  if (state.toDoList.toDoList.length) {
    for (let i = 0; i < state.toDoList.toDoList.length; i++) {
      if (~state.toDoList.toDoList[i].name.toLowerCase().indexOf(state.search.searchText.toLowerCase()) && state.toDoList.toDoList[i].isCompleted) {
...
export const getActiveCount = state => {
  activeCount = 0
  for (let i = 0; i < state.toDoList.toDoList.length; i++) {
    if (!state.toDoList.toDoList[i].isCompleted && !state.toDoList.toDoList[i].important) {
      activeCount++
    }
  }
  return activeCount
