...
export default function toDoItemsReducer(
  ...
  switch (type) {
    ...
    case ADD_NEW_VIDEO:
      const indexItem = state.toDoList.findIndex(
    ...
    case TOOGLE_STATE_IMPORTANT_ITEM:
      const indexImportantItem = state.toDoList.findIndex(
        item => item.id === payload
      );
      state.toDoList[indexImportantItem].important = !state.toDoList[
        indexImportantItem
      ].important;
      return {
        ...state
    ...
    case TOOGLE_STATE_COMPLETED_ITEM:
      const indexCompletedItem = state.toDoList.findIndex(
        item => item.id === payload
      );
      state.toDoList[indexCompletedItem].isCompleted = !state.toDoList[
        indexCompletedItem
      ].isCompleted;
      return {
        ...state
    ...
    case SAVE_CHANGED_ITEM:
      const indexIsEditingItem = state.toDoList.findIndex(
        item => item.id === payload.id
      );
    ...
    case REMOVE_ITEM_FROM_TODO_LIST:
      ...
      const indexToRemove = state.toDoList.findIndex(
        item => item.id === payload
      );
